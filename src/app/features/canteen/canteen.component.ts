...
@Component({
  ...
  template: `<div class="space-y-4"><div class="card"><h2 class="text-xl font-semibold mb-3">Canteen</h2><form [formGroup]="form" (ngSubmit)="create()" class="grid md:grid-cols-2 gap-3"><app-form-input label="Item" [control]="form.controls.item"></app-form-input><app-form-input label="Price" [control]="form.controls.price"></app-form-input><button class="btn-primary md:col-span-2">Save</button></form></div><div class="card"><app-data-table [rows]="rows()" [columns]="columns"></app-data-table></div></div>`
})
export class CanteenComponent {
  private readonly fb = inject(FormBuilder);
  private readonly service = inject(CanteenService);
  readonly columns = ['item', 'price'];
  readonly rows = signal<Record<string, string | number>[]>([]);
  readonly form = this.fb.nonNullable.group({item: [''], price: ['']});
